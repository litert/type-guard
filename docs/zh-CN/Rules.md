...
使用 `$.enum` 修饰可以方便地对其修饰的规则进行枚举值断言，比如：

如果要对 `"aaa" | "bbb" | "ccc"` 类型进行断言检测，那么此前版本中会写作：
...
1. 枚举值是字面量，里面的字符串里的特殊表达式也将被当作普通字符串处理，比如 `["$.enum", "==aaa"]` 匹配的值是 `==aaa` 而不是 `aaa`。

2. 枚举值列表不能为空，否则会报错。

3. 枚举值列表的元素类型不要求一致，可以多种类型组合，但元素值必须是以下类型之一：
